...
export type SpinPhysicsConfig = {
  ...
  /**
   * Nếu set, animation chạy theo thời gian cố định (ease-out) thay vì friction physics.
   */
  durationMs?: number;
};
...
export function startSpinAnimation(params: StartSpinAnimationParams): SpinAnimationController {
  ...
  if (physics.durationMs && params.targetSegmentId) {
    return startTimedSpinAnimation(params, physics.durationMs, physics.extraSpins);
  }
...
function startTimedSpinAnimation(
  params: StartSpinAnimationParams,
  durationMs: number,
  extraSpins: number,
): SpinAnimationController {
  const targetRotationDeg =
    params.targetSegmentId !== undefined
      ? buildLandingRotation(params.model, params.targetSegmentId, {
          startRotationDeg: params.fromRotationDeg,
          extraSpins,
          pointerOffsetDeg: params.model.pointerAngleDeg,
        })
      : params.fromRotationDeg;
